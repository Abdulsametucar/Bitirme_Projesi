...
def test_horizontal_line(video_path, frame_pos):
    ...
    # Ustten iceri katlandigi icin ust yarimlik kismi kontrol et
    # DIS kenarlari haric tutmak icin y'den %10 asagi, yanlardan %10 iceri
    top_y = int(y + h * 0.1)
    ...
    # Yatay cizgileri aramak (genisligin en az %30'u kadar uzun olmali)
    min_len = w * 0.3
    lines = cv2.HoughLinesP(sub_edges, 1, np.pi/180, threshold=40, 
                            minLineLength=min_len, maxLineGap=30)
                            
    # Dikey cizgileri elemek icin aciyi kontrol et (yatay olmali)
    horizontal_count = 0
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
            angle = np.abs(np.arctan2(y2 - y1, x2 - x1) * 180.0 / np.pi)
            if angle < 15 or angle > 165: # +/- 15 derece yatay toleransi
                horizontal_count += 1
...
c_ok = test_horizontal_line('c:/Users/Samet Uçar/Desktop/Bitirme_Projesi/havlu_takip_projesi/data/Ornek_Video.mp4', 450)
